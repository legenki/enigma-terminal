...
``hashlib.new("ripemd160")`` is unavailable wherever OpenSSL 3 ships without the
legacy provider, which is common on modern distributions. Bitcoin's HASH160 needs
it, so the game carries its own implementation and falls back to it transparently.
"""
...
def ripemd160(message: bytes) -> bytes:
    ...
    for offset in range(0, len(padded), 64):
        ...
        for j in range(80):
            rnd = j // 16
            t = _rol(
                (al + _f(j, bl, cl, dl) + block[_RL[rnd][j % 16]] + _KL[rnd]) & _MASK,
                _SL[rnd][j % 16],
            )
            t = (t + el) & _MASK
            al, bl, cl, dl, el = el, t, bl, _rol(cl, 10), dl
            t = _rol(
                (ar + _f(79 - j, br, cr, dr) + block[_RR[rnd][j % 16]] + _KR[rnd]) & _MASK,
                _SR[rnd][j % 16],
            )
            t = (t + er) & _MASK
