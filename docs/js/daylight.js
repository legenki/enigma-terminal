...
const DAY = [
  { h: 0,      bg: '#141413', surface: '#1f1e1d', sunken: '#1a1918', line: '#2f2e2b', ink: '#faf9f5', soft: '#a4a29a', accent: '#e08a6a', btn: '#e08a6a', info: '#85b0d8', warn: '#e0b060', danger: '#ef8880', ok: '#9fb37e', muted: '#8a8880', tint: '#2a2320' },
  { h: 5.3333, bg: '#16161a', surface: '#212126', sunken: '#1c1c20', line: '#31313a', ink: '#f7f6f2', soft: '#a2a1ac', accent: '#e08a6a', btn: '#e08a6a', info: '#85b0d8', warn: '#e0b060', danger: '#ef8880', ok: '#9fb37e', muted: '#8a8880', tint: '#2b2429' },
  { h: 7.3333, bg: '#faf8f2', surface: '#ffffff', sunken: '#f2efe6', line: '#e6e2d6', ink: '#141413', soft: '#6d6b63', accent: '#d97757', btn: '#b85736', info: '#3d6a94', warn: '#8a5a12', danger: '#a32b2b', ok: '#4f6136', muted: '#6e6a61', tint: '#fbf0eb' },
  { h: 12,     bg: '#faf9f5', surface: '#ffffff', sunken: '#f3f1ea', line: '#e8e6dc', ink: '#141413', soft: '#6d6b63', accent: '#d97757', btn: '#b85736', info: '#3d6a94', warn: '#8a5a12', danger: '#a32b2b', ok: '#4f6136', muted: '#6e6a61', tint: '#fbf0eb' },
  { h: 15.5,   bg: '#f9f5ec', surface: '#fffdf8', sunken: '#f2ece0', line: '#e7e0d0', ink: '#161513', soft: '#6f6a5e', accent: '#d06a48', btn: '#ad4f30', info: '#3d6a94', warn: '#8a5a12', danger: '#a32b2b', ok: '#4f6136', muted: '#6e6a61', tint: '#fbeee6' },
  { h: 17.3333,bg: '#f7f1e5', surface: '#fffcf6', sunken: '#efe8d9', line: '#e4dcc9', ink: '#181613', soft: '#6f695b', accent: '#c96343', btn: '#a54a2c', info: '#3d6a94', warn: '#8a5a12', danger: '#a32b2b', ok: '#4f6136', muted: '#6e6a61', tint: '#fbebe1' },
  { h: 20,     bg: '#1a1918', surface: '#242322', sunken: '#1f1e1d', line: '#35332f', ink: '#faf9f5', soft: '#a6a49b', accent: '#e08a6a', btn: '#e08a6a', info: '#85b0d8', warn: '#e0b060', danger: '#ef8880', ok: '#9fb37e', muted: '#8a8880', tint: '#2d2521' },
  { h: 22,     bg: '#161615', surface: '#211f1e', sunken: '#1b1a19', line: '#312f2c', ink: '#faf9f5', soft: '#a4a29a', accent: '#e08a6a', btn: '#e08a6a', info: '#85b0d8', warn: '#e0b060', danger: '#ef8880', ok: '#9fb37e', muted: '#8a8880', tint: '#2b2320' },
  { h: 24,     bg: '#141413', surface: '#1f1e1d', sunken: '#1a1918', line: '#2f2e2b', ink: '#faf9f5', soft: '#a4a29a', accent: '#e08a6a', btn: '#e08a6a', info: '#85b0d8', warn: '#e0b060', danger: '#ef8880', ok: '#9fb37e', muted: '#8a8880', tint: '#2a2320' },
];

export const TOKENS = ['bg', 'surface', 'sunken', 'line', 'ink', 'soft', 'accent', 'btn',
  'info', 'warn', 'danger', 'ok', 'muted', 'tint'];

const hex = (s) => [parseInt(s.slice(1, 3), 16), parseInt(s.slice(3, 5), 16), parseInt(s.slice(5, 7), 16)];
const str = (c) => '#' + c.map((v) => Math.round(Math.min(255, Math.max(0, v))).toString(16).padStart(2, '0')).join('');
...
export function luminance(colour) {
  const c = hex(colour).map((v) => v / 255)
...
export function crossesCut(from, to) {
  const spans = (a, b, mark) => (b >= a ? a < mark && mark <= b : a < mark || mark <= b);
...
export class Daylight {
  ...
  paint(palette) {
    this.shown = palette;
    for (const key of TOKENS) this.root.style.setProperty(`--${key}`, palette[key]);
    // The filled action changes side through the day, so its label follows the
    // fill rather than being pinned to white.
    this.root.style.setProperty('--btn-ink', luminance(palette.btn) < 0.3 ? '#ffffff' : '#1f1e1d');
  ...
  advance(now) {
    ...
    if (!this.tween) { this.paint(target); return; }
