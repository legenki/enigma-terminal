...
].map((hex) => [parseInt(hex.slice(0, 8), 16) | 0, parseInt(hex.slice(8), 16) | 0]);
...
export function sha512(message) {
  ...
  for (let i = 0; i < 8; i++) { hh[i * 2] = IV512[i][0]; hh[i * 2 + 1] = IV512[i][1]; }
  ...
  for (let offset = 0; offset < blocks; offset += 128) {
    ...
    let ah = hh[0], al = hh[1], bh = hh[2], bl = hh[3], ch = hh[4], cl = hh[5];
    let dh = hh[6], dl = hh[7], eh = hh[8], el = hh[9], fh = hh[10], fl = hh[11];
    let gh = hh[12], gl = hh[13], hih = hh[14], hil = hh[15];
    ...
    for (let i = 0; i < 80; i++) {
      ...
      let t2h = (S0h + majh + ((t2l / 0x100000000) | 0)) | 0;

      hih = gh; hil = gl; gh = fh; gl = fl; fh = eh; fl = el;
      let lo = (dl >>> 0) + (t1l >>> 0);
    ...
    add(8, eh, el); add(10, fh, fl); add(12, gh, gl); add(14, hih, hil);
  }
...
export function pbkdf2Sha512(password, salt, iterations, keyLength) {
  ...
  for (let block = 1; block <= blocks; block++) {
    ...
    out.set(acc.subarray(0, Math.min(64, keyLength - (block - 1) * 64)), (block - 1) * 64);
  }
  return out;
}
...
export function ripemd160(message) {
  ...
  for (let offset = 0; offset < blocks; offset += 64) {
    ...
    for (let j = 0; j < 80; j++) {
      ...
      t = (ar + rmdF(79 - j, br, cr, dr) + x[RR[round][j % 16]] + KR[round]) | 0;
  ...
  const outView = new DataView(out.buffer);
  [h0, h1, h2, h3, h4].forEach((value, i) => outView.setInt32(i * 4, value, true));
  return out;
}
