...
import { N, bigIntToBytes32, bytesToBigInt, publicKey } from './secp256k1.js';
...
export class ExtendedKey {
  constructor(key, chainCode, depth = 0, parentFingerprint = new Uint8Array(4), childNumber = 0) {
    this.key = key;
  ...
  child(index) {
    ...
    const payload = hardened ? concat(new Uint8Array([0]), this.key) : this.publicKey;
  ...
  derivePath(path) {
    return path
      ...
      .reduce((node, part) => {
        ...
        if (!Number.isInteger(number)) throw new Error(`bad derivation path segment: ${part}`);
  ...
  toWif() {
    return base58CheckEncode(concat(new Uint8Array([0x80]), this.key, new Uint8Array([1])));
  }
}

const p2pkh = (pub) => base58CheckEncode(concat(new Uint8Array([0x00]), hash160(pub)));
...
export const PURPOSES = [
  { purpose: 44, label: 'Legacy P2PKH', encode: p2pkh, version: new Uint8Array([0x04, 0x88, 0xad, 0xe4]) },
  { purpose: 49, label: 'Nested SegWit', encode: p2shP2wpkh, version: new Uint8Array([0x04, 0x9d, 0x78, 0x78]) },
  { purpose: 84, label: 'Native SegWit', encode: p2wpkh, version: new Uint8Array([0x04, 0xb2, 0x43, 0x0c]) },
];

/** Full chain: mnemonic -> seed -> master xprv -> account keys -> addresses. */
export function deriveWallet(mnemonic, { passphrase = '', account = 0, index = 0 } = {}) {
